...
def hdl_analysis_2(test_value):
    ...
    This function implements the /hdl_analysis/<test_value> route of the server.
    Rather than receiving the HDL value to analyze via JSON in a POST request,
    this route uses a variable URL in which to receive the integer value of
    the HDL value.  The variable URL component is changed into an integer and
    then sent to the blood_analysis.check_HDL function and receives a string in
    return.  This string is then returned to the requestor.
...
def validate_post_input(in_data, expected_keys, expected_types):
    ...
    the keywords and data types sent to the server are those that were expected.
    If an expected keyword is missing, or a value data type does not match
    the expected, an error message is returned along with a 400 status code.
    If the keys and data types do match, True is return with a 200 status code.
    ...
    Args:
        ...
        expected_types (list of types):  the expected data types for the
                                           expected keys.  Should be in the same
                                           order as the expected keys.
